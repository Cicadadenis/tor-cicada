

import os
import sys
import getopt
from requests import get
import subprocess
import time
import signal
from stem import Signal
from stem.control import Controller
from packaging import version


VERSION = "3.1.1"

IP_API = "https://api.ipify.org/?format=json"

LATEST_RELEASE_API = "https://github.com/Cicadadenis/tor-cicada/blob/main/tor-cicada.py"


class bcolors:

    BLUE = '\033[94m'
    GREEN = '\033[92m'
    RED = '\033[31m'
    YELLOW = '\033[93m'
    FAIL = '\033[91m'
    ENDC = '\033[0m'
    BOLD = '\033[1m'
    BGRED = '\033[41m'
    WHITE = '\033[37m'
    

def t():
    current_time = time.localtime()
    ctime = time.strftime('%H:%M:%S', current_time)
    return '[' + ctime + ']'


def sigint_handler(signum, frame):
    print("User interrupt ! shutting down")
    stop_torghost()


def logo():
    print(bcolors.RED + bcolors.BOLD)
    print("""
 _____           ____ _               _       
|_   _|__  _ __ / ___(_) ___ __ _  __| | __ _ 
  | |/ _ \| '__| |   | |/ __/ _` |/ _` |/ _` |
  | | (_) | |  | |___| | (_| (_| | (_| | (_| |
  |_|\___/|_|   \____|_|\___\__,_|\__,_|\__,_|

	{V} - github.com/Cicadadenis/

    """.format(V=VERSION))
    print(bcolors.ENDC)
    
    
    

def ip():
    while True:
        try:
            jsonRes = get(IP_API).json()
            ipTxt = jsonRes["ip"]
        except:
            continue
        break
    return ipTxt


def check_root():
    if os.geteuid() != 0:
        print("Вы должны быть root; Скажи волшебное слово 'sudo'")
        sys.exit(0)


signal.signal(signal.SIGINT, sigint_handler)

TorrcCfgString = \
    """
VirtualAddrNetwork 10.0.0.0/10
AutomapHostsOnResolve 1
TransPort 9040
DNSPort 5353
ControlPort 9051
RunAsDaemon 1
"""

resolvString = 'nameserver 127.0.0.1'

Torrc = '/etc/tor/torghostrc'
resolv = '/etc/resolv.conf'

logo()

os.system('sudo cp /etc/resolv.conf /etc/resolv.conf.bak')
if os.path.exists(Torrc) and TorrcCfgString in open(Torrc).read():
    print(t() + ' Файл Torrc уже настроен')
else:

    with open(Torrc, 'w') as myfile:
        print(t() + ' Запись файла torcc ')
        myfile.write(TorrcCfgString)
        print(bcolors.GREEN + '[done]' + bcolors.ENDC)
if resolvString in open(resolv).read():
    print(t() + 'Файл DNS resolv.conf уже настроен')
else:
    with open(resolv, 'w') as myfile:
        print(t() + ' Настройка файла DNS resolv.conf.. '),
        myfile.write(resolvString)
        print(bcolors.GREEN + '[done]' + bcolors.ENDC)
 	
 	
 	
os.system('sudo systemctl stop tor')
os.system('sudo fuser -k 9051/tcp > /dev/null 2>&1')
print(bcolors.GREEN + '[done]' + bcolors.ENDC)
print(t() + ' Запуск нового демона tor '),
os.system('sudo -u debian-tor tor -f /etc/tor/torghostrc > /dev/null'
          )
print(bcolors.GREEN + '[done]' + bcolors.ENDC)
print(t() + ' настройка правил iptables'),

iptables_rules = \
    """
NON_TOR="192.168.1.0/24 192.168.0.0/24"
TOR_UID=%s
TRANS_PORT="9040"
iptables -F
iptables -t nat -F

iptables -t nat -A OUTPUT -m owner --uid-owner $TOR_UID -j RETURN
iptables -t nat -A OUTPUT -p udp --dport 53 -j REDIRECT --to-ports 5353
for NET in $NON_TOR 127.0.0.0/9 127.128.0.0/10; do
 iptables -t nat -A OUTPUT -d $NET -j RETURN
done
iptables -t nat -A OUTPUT -p tcp --syn -j REDIRECT --to-ports $TRANS_PORT
	iptables -A OUTPUT -m state --state ESTABLISHED,RELATED -j ACCEPT
for NET in $NON_TOR 127.0.0.0/8; do
 iptables -A OUTPUT -d $NET -j ACCEPT
done
iptables -A OUTPUT -m owner --uid-owner $TOR_UID -j ACCEPT
iptables -A OUTPUT -j REJECT
""" \
    % subprocess.getoutput('id -ur debian-tor')
os.system(iptables_rules)
print(bcolors.GREEN + '[done]' + bcolors.ENDC)
print(t() + 'Получение IP...')
print(t() + ' ТЕКУЩИЙ IP : ' + bcolors.GREEN + ip() + bcolors.ENDC)
def stop_torghost():
    print(bcolors.RED + t() + 'ОСТАНОВКА TorCicada' + bcolors.ENDC)
    print(t() + ' Очистка iptables, сброс на значения по умолчанию'),
    os.system('mv /etc/resolv.conf.bak /etc/resolv.conf')
    IpFlush = \
        """
	iptables -P INPUT ACCEPT
	iptables -P FORWARD ACCEPT
	iptables -P OUTPUT ACCEPT
	iptables -t nat -F
	iptables -t mangle -F
	iptables -F
	iptables -X
	"""
    os.system(IpFlush)
    os.system('sudo fuser -k 9051/tcp > /dev/null 2>&1')
    print(bcolors.GREEN + '[done]' + bcolors.ENDC)
    print(t() + ' Перезапуск сетевого менеджера'),
    os.system('service network-manager restart')
    print(bcolors.GREEN + '[done]' + bcolors.ENDC)
    print(t() + ' Получение IP...')
    time.sleep(3)
    print(t() + ' ТЕКУЩИЙ IP : ' + bcolors.GREEN + ip() + bcolors.ENDC)
